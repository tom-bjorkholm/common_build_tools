...
def test_dynamic_package_dependencies_skips_internal_and_duplicates() -> None:
    ...
    info = make_build_information(
        ...
        package_information=[
            make_package_information(
                package_folder=Path('/tmp/pkg-core'),
                name='pkg-core',
                dependencies=['requests>=2.0', 'pkg-utils>=1.0']
            ),
            make_package_information(
                package_folder=Path('/tmp/pkg-utils'),
                name='pkg-utils',
...
def test_setup_build_environment_runs_expected_steps(
    ...
    monkeypatch.setattr(
        setup_build_environment,
        '_create_or_recreate_venv',
        lambda _cmd, force_recreate: events.append(
            f'create_venv:{force_recreate}'
        )
    )
    monkeypatch.setattr(
        setup_build_environment,
...
def test_setup_build_environment_cmd_exits_zero(
    ...
    monkeypatch.setattr(
        setup_build_environment,
        'setup_build_environment',
        lambda python_name, force_recreate, build_spec, build_information:
        called.append(f'{python_name}:{force_recreate}')
    )
