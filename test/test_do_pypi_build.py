...
def test_clean_fail_returns_code(monkeypatch: pytest.MonkeyPatch,
                                 tmp_path: Path,
                                 capsys: pytest.CaptureFixture[str]) -> None:
    """Test helper prints message and returns code on clean_build failure."""
    info = make_build_information(tmp_path)
    monkeypatch.setattr(do_pypi_build, 'clean_build', lambda **_kwargs: 4)
    result = do_pypi_build._run_clean_build_or_fail(python_name='python3.12',
                                                    run_name='First run',
...
def test_pypi_build_stops_on_fail(monkeypatch: pytest.MonkeyPatch,
                                  tmp_path: Path) -> None:
    """Test do_pypi_build returns first non-zero clean_build result."""
    info = make_build_information(tmp_path)
    monkeypatch.setattr(do_pypi_build, 'clean_build', lambda **_kwargs: 2)
    result = do_pypi_build.do_pypi_build(python_name='python3.12',
                                         twine_upload=False,
                                         build_spec=BuildSpec(),
                                         build_information=info)
    assert result == 2
...
def test_pypi_build_no_upload(monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
                              capsys: pytest.CaptureFixture[str]) -> None:
    """Test do_pypi_build prints no-upload message when twine is disabled."""
    info = make_build_information(tmp_path)
    monkeypatch.setattr(do_pypi_build, 'clean_build', lambda **_kwargs: 0)
    result = do_pypi_build.do_pypi_build(python_name='python3.12',
                                         twine_upload=False,
                                         build_spec=BuildSpec(),
                                         build_information=info)
...
def test_pypi_build_runs_upload(monkeypatch: pytest.MonkeyPatch,
                                tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(do_pypi_build, 'run_command', lambda cmd,
                        cwd: commands.append(cmd))
    result = do_pypi_build.do_pypi_build(python_name='python3.13',
                                         twine_upload=True,
                                         build_spec=BuildSpec(),
                                         build_information=info)
    assert result == 0
...
def test_pypi_build_cmd_exits(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(do_pypi_build, 'do_pypi_build', lambda *_args,
                        **_kwargs: 7)
